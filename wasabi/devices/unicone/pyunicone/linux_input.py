...
# Wrappers around the ioctls
def iocGetName(fd):
    """Gets the name of an event device"""
    return ioctl(fd, EVIOCGNAME_512, "\0"*512).replace("\0", "")

def iocGetAbs(fd, axis):
    ...
    return dict(zip(( 'value', 'min', 'max', 'fuzz', 'flat' ),values))

def iocGetBits(fd, number):
    """Get the contents of a bitfield, given it's EV_* number. Gets the
       set of EV_* bits if passed zero.
       """
    longSize = struct.calcsize("L")
    buffer = ioctl(fd, EVIOCGBIT_512 + number, "\0"*512)
    longs = struct.unpack("L" * (len(buffer) / longSize), buffer)
    bits = []
    for word in xrange(len(longs)):
        long = longs[word]
        for bit in xrange(8 * longSize):
            if long & 1:
                bits.append(bit + 8 * longSize * word)
            long >>= 1
    return bits

def iocGetBitNames(fd, name):
    """Given an EV_FOO name, returns the list of FOO_* bits
       supported by the device. If 'name' is None, returns the
       list of EV_* bits.
       """
    if name:
