...
#define TRIG_SIZE 10000
#define TRIG_SCALE (TRIG_SIZE / (M_PI/2))
float trig_table[TRIG_SIZE+1];

void init_trig() {
  int i;
  for (i=0;i<TRIG_SIZE;i++)
    trig_table[i] = sin(i/TRIG_SCALE);
  trig_table[TRIG_SIZE] = trig_table[0];
}
...
static void update_color_table() {
  ...
  float luma;
  float one_over_gamma = 1/render.gamma;
  ...
  if (render.color_table_size < required_size) {
    ...
    /* Allocate it to float the size we need now, as we expect our needs to grow. */
...
static inline float fast_sin(float x) {
   ...
   * Use a separate copy of the algorithm that has a built-in
   * inversion if the input is negative.
   */
  if (x<0) {
    /* This is equivalent to 'x = fmod(x, M_PI*2)'. Believe it or not,
     * this turns out to be faster for our particular application, since
     * the values are usually already within the correct range. This was
     * benchmarked at 1.18 times faster than fmod, and 1.49 times faster
     * than 'x = x - ((int)(x / (M_PI*2)))*M_PI*2'. Note that x is always
     * positive, due to the test above.
     */
    while (x < -M_PI*2)
      x += M_PI*2;

    if (x > -M_PI/2) {
      return -trig_table[(int)(x * -TRIG_SCALE)];
    }
    else if (x > -M_PI) {
      return -trig_table[(int)((M_PI + x) * TRIG_SCALE)];
    }
    else if (x > -M_PI*1.5) {
      return trig_table[(int)((x + M_PI) * -TRIG_SCALE)];
    }
    return trig_table[(int)((M_PI*2 + x) * TRIG_SCALE)];
  }
...
void run_iterations(int count) {
  ...
  const float xcenter = count_width / 2.0;
  const float ycenter = count_height / 2.0;
  const float scale = xcenter / 2.5 * params.zoom;
  ...
  const int fudge_table_size = 128;  /* Must be a power of two */

  float x, y, sine_rotation, cosine_rotation;
  int i, ix, iy;
  guint *p;
  guint d;
  int blur_index, fudge_index;
  float blur_table[blur_table_size];
  float fudge_table[fudge_table_size];
  ...
  /* We can get a high quality image with much less numerical precision than otherwise
   * necessary if we insert random noise, at about the same magnitude as the numerical
   * error in our sine function. This noise isn't noticeable unless the trig table is
   * extremely small, but it keeps the function from visiting a limited subset of the
   * possible points due to the low numerical precision.
   *
   * To avoid having to use the uniform variate inside our iteration loop, we generate
   * a small table of properly scaled random noise that we reuse during the loop.
   */
  for (i=0; i<fudge_table_size; i++)
    fudge_table[i] = uniform_variate() * (trig_table[1] - trig_table[0]);
  fudge_index = 0;
  ...
  for(i=count; i; --i) {
     ...
     *
     * We use fast_sin(), which is faster but lower precision than the default sin().
     * The fudge_table[] is used to conceal the numerical error, as described above.
     */
    x = fast_sin(params.a * point.y) - fast_sin(params.b * point.x + M_PI/2) + fudge_table[fudge_index];
    fudge_index = (fudge_index + 1) & (fudge_table_size - 1);
    y = fast_sin(params.c * point.x) - fast_sin(params.d * point.y + M_PI/2) + fudge_table[fudge_index];
    fudge_index = (fudge_index + 1) & (fudge_table_size - 1);
    point.x = x;
    point.y = y;
