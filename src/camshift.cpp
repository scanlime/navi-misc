...
void interactive_camshift(int n_cameras = 1,
  ...
  while (1) {
	...
	if (box.size.width > 0 && box.size.height > 0) {
	   ...
	   * Draw crosshairs at the point predicted.
	   */
	  const CvMat* prediction = cvKalmanPredict(kalman);
	  if (update_screen)
	    plot_crosshairs(images[i],
			    cvPoint((int)cvmGet(prediction, 0, 0),
	  ...
	  if (uinput_mouse_enabled) {
	    ...
	    float x = cvmGet(prediction, 0, 0);
	    float y = cvmGet(prediction, 1, 0);
	    uinput_mouse_move_subpixel((x-old_x) * 4,
				       (y-old_y) * -4);
	    old_x = x;
	    old_y = y;
	  }
