...
namespace Fyre
{
	class PipelineDrawing : Gtk.DrawingArea
	{
		...
		Gdk.Cursor
		CreateCursor (byte[] data, byte[] mask)
		{
			string datas = System.Text.Encoding.ASCII.GetString (data);
			string masks = System.Text.Encoding.ASCII.GetString (mask);
			Gdk.Color fore = new Gdk.Color (0xff, 0xff, 0xff);
			Gdk.Color back = new Gdk.Color (0x00, 0x00, 0x00);
			Gdk.Pixmap source = Gdk.Pixmap.CreateBitmapFromData (GdkWindow, datas, 20, 20);
			Gdk.Pixmap cmask  = Gdk.Pixmap.CreateBitmapFromData (GdkWindow, masks, 20, 20);
