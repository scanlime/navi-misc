...
int main(int argc, char **argv) {
  ...
  fd_set rfd,wfd;
  int device= argv[1] ? atoi(argv[1]) : 5;
  int addr,value;
  ...
  while (1) {
    ...
    if (FD_ISSET(1,&rfd)) {
      scanf("%d %d",&addr,&value);

      memset(&buffer,0,sizeof(buffer));
      buffer[0] = (device << 5)        | (addr >> 4);
      buffer[1] = (device << 5) | 0x10 | (addr & 0x0F);
      buffer[2] = (device << 5) | 0x10 | (value >> 4);
      buffer[3] = (device << 5) | 0x10 | (value & 0x0F);
      write(fd,&buffer,sizeof(buffer));
    }
