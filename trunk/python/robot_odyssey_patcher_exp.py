...
#   4. Enjoy!
#
# Patcher version 1.X, April 11 2009.
...
DEFS = """
        ...
        ; Offset of the .WLD file in memory.
        ; XXX: We should discover this dynamically, it could change

world_data           equ 0x02AC

        ; Object data:
        ;
        ; Most movable objects, including the player and the
        ; cursor the player controls, are represented by
        ; an 8-bit object ID. The world has several tables
        ; of data which are indexed by this ID.

obj_player           equ 0x00   ; Player is moved directly
obj_toolbox          equ 0xE1
obj_chip1            equ 0xE8
obj_chip2            equ 0xE9
obj_chip3            equ 0xEA
obj_chip4            equ 0xEB
...
obj_chip7            equ 0xEE
obj_chip8            equ 0xEF
obj_robot_scanner    equ 0xF4
obj_robot_scanner_2  equ 0xF5
obj_cursor_2         equ 0xFC
obj_cursor           equ 0xFE   ; Moves indirectly when player moves
obj_none             equ 0xFF

w_object_room        equ  world_data + 0x300
w_object_x           equ  world_data + 0x400
w_object_y           equ  world_data + 0x500
w_object_moved_by    equ  world_data + 0x600
w_object_motion_x    equ  world_data + 0x700
w_object_motion_y    equ  world_data + 0x800

        ; ========================
        ; Patch Global Variables
        ; ========================

        ; Our own variables, at the bottom of the stack.
        ; We've reserved 16 bytes total.
...
def asm(code, bits=16, args={}):
    ...
    try:
        ...
        try:
            ...
            lines = ["%4d   %s\n" % (1+i, s) for i, s in enumerate(str.split('\n'))]
...
def blitPatch(exe, fps, kbFast=False, noPIT=False):
    ...
    patch += asm(args=locals(), code=DEFS + """
        ...
        ; Right now, ES and DS are both set to atypical values on
        ; account of the blit.  ES points to the CGA framebuffer, and
        ; DS is the backbuffer. The normal DS is saved in DX, and
        ; ES is reloaded from a global pointer after the blit.
        ;
        ; We store our patch's global data at the bottom of the stack
        ; segment, but we also need access to the game's DS: so set
        ; up ES to point to our normal DS, and DS=SS.
        ...
        ; On the first few calls, do nothing. This will be early in
        ; the game's initialization, before we're even in CGA
        ; mode. After that, do our actual init.

        cmp   byte [init_flag], 10
        jb    nextInitLoop
        jnz   noInitializationNeeded

        xor   ax, ax              ; Initialize the mouse driver
        int   0x33
...
noInitializationNeeded:
        ...
        ; This is a simple event loop that we run while waiting to
        ; draw the next frame. In effect, we're stealing control from
        ; the game's real main loop, and returning control only when
        ; it's time for another frame to elapse.

        xor   ax, ax              ; Show mouse cursor
        inc   al
        int   0x33
...
mainLoop:
        ...
        cmp   bx, %(numTicks)d    ; Have enough ticks passed?
        jb    timerNotElapsed
localNextFrame:
        jmp   nextFrame
timerNotElapsed:

        ; Optional, update keyboard state. If there are keys waiting
        ; in the buffer, return control back to the game.
        ...
        ; Update mouse state. If there's a click, handle it.
        ; We need to use Int 33 AX=3 to poll the mouse's current
        ; state, and we also need to use AX=5 to check for any
        ; clicks we may have missed.

        mov   ax, 3               ; Get mouse status
        int   0x33
        shr   cx, 2               ; Scale X to wide-pixels, adjust center
        sub   cx, 3
        mov   al, 177 + 8         ; Invert Y, adjust sprite center
        sub   al, dl
        mov   [click_x], cl       ; Save position
        mov   [click_y], al
        or    bl, bl
        jnz   buttonDown          ; Mouse button is still down

        mov   ax, 5               ; Get button press data
        xor   bx, bx              ; Left button
        int   0x33
        or    bx, bx
        jnz   buttonDown

        ; Mouse button is not down. Reset mouse interaction state.

        mov   byte [click_obj], obj_none
        jmp   mouseDone

buttonDown:

        ; XXX: Experimental, move the closest object
        ;      in this room to the cursor position.

        mov   al, [click_obj]     ; Already dragging an object.
        cmp   al, obj_none        ; Keep dragging it. Otherwise, search for the
        jnz   haveObject          ; closest object near the mouse in this room.

        mov   si, 0               ; Loop iterator
        mov   di, obj_none        ; Closest object
        mov   cx, 0x30            ; Distance of closest object (init to max dist.)
objectLoop:

        mov   al, [ES:w_object_room]       ; Get player's current room
        mov   ah, [ES:w_object_room + si]  ; Get object SI's room
        xor   al, ah
        jnz   nextObject

        mov   al, [ES:w_object_x + si]     ; 16-bit absolute value of X distance
        xor   ah, ah
        mov   dl, [click_x]
        xor   dh, dh
        sub   dx, ax
        ...
        neg   dx
abs_x:
        mov   al, [ES:w_object_y + si]     ; 16-bit absolute value of Y distance
        xor   ah, ah
        mov   bl, [click_y]
        xor   bh, bh
        sub   bx, ax
        ...
        neg   bx
abs_y:
        add   bx, dx              ; Approximate distance: abs(x) + abs(y)

        cmp   cx, bx              ; Skip if this isn't closer.
                                  ; If there's a tie, we must take the
                                  ; lowest numbered object. This resolves
                                  ; the issue with duplicate object IDs
                                  ; for some entities (robots, cursor)
        jbe   nextObject
        mov   cx, bx              ; Save as closest object
        mov   di, si
...
nextObject:
        ...
        jnz   objectLoop
        mov   ax, di
        mov   [click_obj], al     ; Save search result


haveObject:
        mov   al, [click_obj]     ; Move the object
        cmp   al, obj_none
        jz    mouseDone
        xor   ah, ah
        mov   si, ax
        mov   al, [click_x]
        mov   [ES:w_object_x + si], al
        mov   al, [click_y]
...
def commentPatch(exe, opts):
    ...
    # global variables by other patches, so make sure that space is
    # iniitalized to zero.
    ...
    print "Saving comment at 0x%x" % (len(padding) + stackOffset)
    exe[stackOffset] = padding + comment
...
class Patchable:
    ...
    def _growToLength(self, l):
        if l > len(self._array):
            self._array.extend([0] * (l - len(self._array)))
    ...
    def __setitem__(self, x, value):
        if type(value) in (int, long):
            ...
            self._growToLength(self._offset + x + 1)
            self._array[x + self._offset] = value

        elif type(value) is str:
            self._growToLength(self._offset + x + len(value))
