...
parser.add_option('-m','--mask',dest="mask",action="store_true",
        help="mask path(s)")
...
parser.add_option('-u','--unmask',dest="unmask",action="store_true",
        help="unmask path(s)")
...
#
import Trough.Select
from Trough.PlayList import PlayList
...
if options.add:
    ...
    playlist.getGroupList().addPaths(arguments)
    playlist.dump()
    sys.exit(0)
if options.remove:
    print 'removing ',arguments
    playlist.getGroupList().removePaths(arguments)
    playlist.dump()
    sys.exit(0)
if options.breakup:
    print 'breaking ',arguments
    playlist.getGroupList().breakGroups(arguments)
    playlist.dump()
    sys.exit(0)

# At some point, this may be replaced by code that actually plays something
#
p = playlist.getGroupList()

for x in Trough.Select.Select(p):
    print x.getPath()
