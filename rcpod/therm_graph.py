...
import gtk, time, threading, pyrcpod, i2c
...
class AnalogUpdaterThread(threading.Thread):
	...
	self.i2cb = i2c.i2c_bus(self.rcpod)
	self.therm = i2c.therm(self.i2cb)
	self.running = True
        self.start()

    def run(self):
        while self.running:
	    config = self.therm.read(0x48, 0x01)[0]
	    if (config >> 6) & 0x01:
                # Perform an analog reading, using the array helpers
	        temps = self.therm.read(0x48, 0x00)
	        for temp in temps:
	            if (temp >> 7) & 0x01:
		        temp |= 0xffffff00
