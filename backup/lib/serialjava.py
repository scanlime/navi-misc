...
#
# (C) 2002 modified slightly by Wouter van Ooijen <wouter@voti.nl>
# original license.txt applies
...
class Serial(serialutil.FileLike):
    def __init__(self,
        ...
        if timeout >= 0:
            self.sPort.enableReceiveTimeout(int(timeout*1000))
        else:
    ...
    def sendBreak(self, duration = None):
        if not self.sPort: raise portNotOpenError
        if duration == None:
           duration = 100 * ( 1 / self.baudrate )
        self.sPort.sendBreak(int(1000*duration))
