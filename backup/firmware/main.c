...
#define NUM_RESONATION_HALFCYCLES  3      /* Number of half-cycles used to excite the resonator */
#define INTEGRATION_SAMPLES        8      /* Number of samples to integrate-
...
int integrateSample(unsigned char transmitter, unsigned char receiver) {
  signed int i=0, q=0;
  int numsamples = INTEGRATION_SAMPLES;
...
int sensorSample(unsigned char transmitter, unsigned char receiver, unsigned char phase) {
   ...
   */
  halfcycles = NUM_RESONATION_HALFCYCLES;
  do {
    PORTB ^= transmitter;
    NOP NOP NOP;
    NOP NOP NOP NOP NOP;
