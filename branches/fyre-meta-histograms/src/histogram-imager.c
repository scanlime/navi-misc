...
histogram_imager_update_image (HistogramImager *self)
{
	...
	if (oversample > 1) {
	    ...
	    pixel_p = (guint32*) gdk_pixbuf_get_pixels (self->image);
	    for (x=0; x<512; x++) {
		v = log(hist[x]) / log(self->width*self->height) * self->height * 0.5;

		for (y=0; y<(self->height-10) && y<=v; y++)
		    pixel_p[10 + x + ((10+y)*self->width)] = 0xFF0000;
	    }
