...
static void histogram_imager_init(HistogramImager *self);
...
GType histogram_imager_get_type(void) {
  ...
  if (!dj_type) {
    static const GTypeInfo dj_info = {
      ...
      0,
      (GInstanceInitFunc) histogram_imager_init,
    };
...
static void histogram_imager_generate_color_table(HistogramImager *self) {
  ...
  printf("pixel scale %f, usable density %d\n", pixel_scale, usable_density);
