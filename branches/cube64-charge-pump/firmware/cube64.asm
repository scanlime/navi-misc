	...
	;; (with 1k pullup) is on RA3, the N64's bidirectional data line
	;; is on RA2. All of PORTB is used to drive a charge pump powering
	;; the rumble motor. The PIC16F84 was chosen due to its relatively high
	...
	#define N64_PIN		PORTA, 2
	#define N64_TRIS	TRISA, 2
	#define	GAMECUBE_PIN	PORTA, 3
	#define	GAMECUBE_TRIS	TRISA, 3
...
n64_rx_bus
	...
	;; Wait for an N64 command. We spend most of our time here, so take the
	;; opportunity to pat the watchdog and to pump the rumble power supply.
	;; This is really ugly, but it's extremely important that our latency
	;; in detecting the beginning of a command is as low as possible.
n64_rx_command
	movlw	.1
	movwf	byte_count
n64_poll_loop
	btfss	N64_PIN
	goto	n64_start_command_rx+2

	clrwdt
	btfss	N64_PIN
	goto	n64_start_command_rx+2

	comf	charge_pump_state, f
	btfss	N64_PIN
	goto	n64_start_command_rx+2

	movf	charge_pump_state, w
	btfss	N64_PIN
	goto	n64_start_command_rx+2
