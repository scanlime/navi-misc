;################################################################################
;
; descript.asm - USB descriptors for the uvswitch project
;
...
StartDevDescr
	...
	retlw	UVSWITCH_VENDOR_ID & 0xFF	; idVendor
	retlw	UVSWITCH_VENDOR_ID >> 8
	retlw	UVSWITCH_PRODUCT_ID & 0xFF	; idProduct
	retlw	UVSWITCH_PRODUCT_ID >> 8
	retlw	UVSWITCH_PROTOCOL_VERSION & 0xFF	; bcdDevice
	retlw	UVSWITCH_PROTOCOL_VERSION >> 8
...
Endpoint1In
	...
	retlw	0x08		; max packet size (1 byte) low order byte
	retlw	0x00		; max packet size (1 byte) high order byte
	retlw	0x80		; polling interval (128ms)
EndConfig1
