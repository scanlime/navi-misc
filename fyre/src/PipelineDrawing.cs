...
namespace Fyre
{
	class PipelineDrawing : Gtk.DrawingArea
	{
		...
		Gdk.Cursor HandOpenCursor
		{
			get {
				if (hand_open_cursor == null)
					hand_open_cursor = CreateCursor (hand_open_data, hand_open_mask, 20, 20);
		...
		Gdk.Cursor HandClosedCursor
		{
			get {
				if (hand_closed_cursor == null)
					hand_closed_cursor = CreateCursor (hand_closed_data, hand_closed_mask, 20, 20);
		...
		Gdk.Cursor
		CreateCursor (byte[] data, byte[] mask, int width, int height)
		{
			string datas = System.Text.Encoding.Default.GetString (data);
			string masks = System.Text.Encoding.Default.GetString (mask);
			Gdk.Color fore = new Gdk.Color (0xff, 0xff, 0xff);
			Gdk.Color back = new Gdk.Color (0x00, 0x00, 0x00);
			Gdk.Pixmap source = Gdk.Pixmap.CreateBitmapFromData (GdkWindow, datas, width, height);
			Gdk.Pixmap cmask  = Gdk.Pixmap.CreateBitmapFromData (GdkWindow, masks, width, height);
		...
		OnExposeEvent (Gdk.EventExpose ev)
		{
			...
			Gdk.GC gc = Style.DarkGC (Gtk.StateType.Prelight);
