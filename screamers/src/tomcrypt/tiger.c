...
INLINE static void ourround(ulong64 *a, ulong64 *b, ulong64 *c, ulong64 x, int mul)
{
    ulong64 tmp;
    tmp = (*c ^= x); 
...
{
   ourround(a,b,c,x[0],mul); 
   ourround(b,c,a,x[1],mul); 
   ourround(c,a,b,x[2],mul); 
   ourround(a,b,c,x[3],mul); 
   ourround(b,c,a,x[4],mul); 
   ourround(c,a,b,x[5],mul); 
   ourround(a,b,c,x[6],mul); 
   ourround(b,c,a,x[7],mul);          
}   
