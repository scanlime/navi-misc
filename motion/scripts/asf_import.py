...
def importObjects(reader):
    ...
    ba = bone.getRestMatrix ().rotationPart ().toEuler ()
    ba = map (str, [ba.x, ba.y, ba.z])
    armObj.addProperty ('root-baxis', ','.join (ba), 'STRING')
    armObj.addProperty ('root-axis', '0,0,0', 'STRING')
    ...
    for set in reader.hierarchy:
        ...
        for bone in children:
            ...
            ba = bones[bone].getRestMatrix ('worldspace').rotationPart ().toEuler ()
            ba = map (str, [ba.x, ba.y, ba.z])
